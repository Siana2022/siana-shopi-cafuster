...
function buildPurchasePayload(order, eventName = 'siana_purchase') {
  ...
  const subscriptionFrequency = purchaseType === 'suscripcion'
    ...
    : null;
  const subscriptionDishes = (order.line_items || [])
    .reduce((sum, i) => sum + (i.quantity || 0), 0) || null;

  const bundleType = getOrderBundleType(order.line_items || []);
  const bundleNamesCo = getOrderBundleNames(order.line_items || []);
  const bundleIdsCo = getOrderBundleIds(order.line_items || []);
  ...
  const payload = {
    ...
    ecommerce: {
      ...
      bundle_type_co:             bundleType,
      bundle_name_co:             bundleNamesCo,
      bundle_id_co:               bundleIdsCo,
      purchase_type_co:           purchaseType,
      subscription_frequency_co:  subscriptionFrequency,
      subscription_dishes_co:     subscriptionDishes,
      items,
    },
...
function getLineItemProperty(item, propName) {
  const p = (item.properties || []).find(x => x && x.name === propName);
  return (p && p.value !== undefined && p.value !== null && p.value !== '') ? p.value : null;
}

function getBundleType(item) {
  return hasLineItemProperty(item, '_isPresetBundleProduct') ? 'predefinido' : 'personalizado';
}

function getOrderBundleType(lineItems) {
  for (const item of lineItems) {
    if (hasLineItemProperty(item, '_isPresetBundleProduct')) return 'predefinido';
  }
  return 'personalizado';
}

function getOrderBundleNames(lineItems) {
  const names = [...new Set(lineItems.map(i => getLineItemProperty(i, '_bundleName')).filter(Boolean))];
  return names.length ? names.join(', ') : null;
}

function getOrderBundleIds(lineItems) {
  const ids = [...new Set(lineItems.map(i => getLineItemProperty(i, '_bundleId')).filter(Boolean))];
...
function buildItems(lineItems, purchaseType, subscriptionFrequency, subscriptionDishes) {
  return lineItems.map(item => ({
    ...
    bundle_id:               getLineItemProperty(item, '_bundleId'),
    bundle_name:             getLineItemProperty(item, '_bundleName'),
